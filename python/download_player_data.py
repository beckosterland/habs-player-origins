...
# Primary geocoder: Photon (OpenStreetMap-based, no strict rate limit, no key needed)
PHOTON_URL = "https://photon.komoot.io/api/"
# Fallback geocoder: Nominatim (requires >=1 req/sec and a descriptive User-Agent)
...
def _geocode_photon(city: str, province: str, country_name: str) -> tuple[str, str]:
    ...
    Includes province/state in the query string so Photon can disambiguate
    cities that share a name across states (e.g. Long Beach NY vs CA).
    Also uses Photon's `osm_tag` filter to restrict results to cities/towns.
    """
    # Build query from most to least specific
...
def geocode_city(city: str, province: str, country_code: str, country_name: str) -> tuple[str, str]:
    ...
    Tries Photon first (faster, more lenient), then falls back to Nominatim.
    ...
    # 2. Fall back to Nominatim if Photon didn't find anything
    if not lat:
        lat, lon = _geocode_nominatim(city, province, country_name, country_code)

    if not lat:
        time.sleep(1.1)  # brief pause before next attempt even on failure
...
def main():
    ...
    output_file = os.path.join(output_dir, f"nhl_active_players_{datestamp}.csv")
    fieldnames = [
